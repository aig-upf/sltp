...
how an abstract actions can be learned by enforcing soundness and
...
accommodated with a simple fix to the SAT theory.
It would be enough to replace the formula \eqref{eq:bridge1} that
links $D1$ and $D2$ with a formula that says that when $s$ and $t$ are
indistinguishable (i.e.\ $\neg D1(s,t)$), for each action $a$ on $s$,
there is an action $b$ on $t$ whose transitions are indistinguishable
from those of $a$ on $s$. To do this, transitions $(s,s')$ in the sample
would need to be tagged with action labels as $(s,a,s')$.
