...
On the other hand, contexts involving non-deterministic actions may be
accommodated with a simple modification of the SAT theory.
Indeed, it would be enough to replace the formula (3) that links $D1$ and $D2$
with a formula that says that when $s$ and $t$ are indistinguishable
(i.e.\ $\neg D1(s,t)$), then for each action $a$ for $s$, there is an action $b$
for $t$ whose sets of possible outcomes are indistinguishable.
