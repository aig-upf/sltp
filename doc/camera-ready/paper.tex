...
if it results in numerical values. %, that we assume to be a non-negative integer.
For example, $H$ and $n(x)$ represent two features in Blocksworld:
$H$ is a boolean feature that tracks whether the gripper is empty, while
$n(x)$ is numerical and tracks the number of blocks above $x$.
...
The number of clauses is bounded by $m^2(2 + b^2 + b + |\F|(b^2 + 1))$. %$m^2 \times b^2 \times |\F|$.
%
\Omit{
...
%Goal predicates are copies $p_G$ of the primitive predicates $p$ that appear
%in the goal and which are evaluated in the goal  \cite{martin-geffner:generalized}.
