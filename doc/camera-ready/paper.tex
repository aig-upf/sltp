...
let $F=\{H,n(x)\}$ be the set of the two features above, and let $s$ be a reachable state in an instance $P$
in $\Q_{clear}$
...
and the concrete  action  $a=\text{Unstack}(A,B)$ in $P$ have the same effect over the features.
...
The number of clauses is bounded by $m^2(3 + b^2 + |\F|(1+b^2))$. %$m^2 \times b^2 \times |\F|$.
%
\Omit{
...
provided by hand.\footnote{$G_F = G_{\sigma}$ can be defined as the DNF formula whose terms 
  correspond to the abstract states over $F_{\sigma}$ that correspond to some goal state in
  the sample. \textcolor{red}{\bf ** Related to previous note **}
  }
...
The cost measure that is used in the experiments is defined next.
...
\begin{enumerate}[{\small$\bullet$}]
  ...
  \item $C \leftarrow \exists R.C,\forall R.C$, first denotes $\{x:\!\exists y[R(x,y) \land C(y)]\}$, %  $x$'s  s.t.\  $R(x,y) \land C(y)$ for some  $y$;
    second $\{x:\forall y[R(x,y)\land C(y)]\}$, %,  $x$'s  s.t. $R(x,y) \rightarrow C(y)$ for all $y$,
...
$n_C$ associated with concept $C$ represents the number of objects in $C^s$. %$|C^s|$.
\textcolor{red}{\bf ** $C: \exists\, on^* . C_x$ also counts block $x$ since * is reflexive! **}
...
\begin{enumerate}[{\small$\bullet$}]
\item For each nullary primitive predicate $p$, a \emph{boolean} feature $b_p$ that is true
  in $s$ iff $p$ is true in $s$.
%
\item For each concept $C$, a \emph{boolean} feature $b_C$, if $|C^s| \in \set{0,1}$ for all
  sampled states $s$, and a \emph{numerical} feature $n_C$ otherwise.
...
\item Numerical features $\textit{dist}(C_1,R{:}C,C_2)$ that represent the smallest
  $n$ such that there are objects $x_1, \ldots, x_n$ satisfying $C_1^s(x_1)$,
  $C_2^s(x_{n})$, and $(R{:}C)^s(x_i,x_{i+1})$ for $i=1,\ldots,n$.
...
\begin{enumerate}[--]
  ...
  \item $n(x): \abs{\exists\,on^* . C_x}$, (number of blocks above block $x$). \footnote{\textcolor{red}{** See note about reflexive closure **}}
\end{enumerate}
...
Since no actions increment the variable $n(x)$, $Q^+_F=Q'_F$.
\textcolor{red}{\bf ** Do we need this comment about $Q^+_F$ **}
The policy that solves the generalized problem $\Q_{clear}$
is obtained from $Q^+_F$ with the planner in 0.46 seconds.
...
Three training instances $P$ are used to learn $F$ from $T_G$
that contains
% 
...
block $x$ being held, and the block $x$ being on block $y$, and the
...
it implements a loop to clear block $x$, followed by a second
loop to clear block $y$, picking then $x$ and placing it on $y$. 
...
translated into $Q^+_F$ which is solved by the FOND planner in 171.92 seconds.
...
is translated into $Q^+_F$ which is solved by the planning in 1.36 seconds.
