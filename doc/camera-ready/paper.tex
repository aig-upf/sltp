...
% \author{Blai  Bonet, G. Frances, H. Geffner} % Alphabetical
...
the same \emph{qualitative effects} on the numerical features $n$. This means: 1)~$p$ becomes true in the state $s'$
...
instances $P$ in $\Q$ in terms of abstract actions that operate at the level of the  features:
...
If we say that  action $a$  \emph{instantiates} an abstract action $\bar{a}$ in a state $s$
and that $\bar{a}$ captures $a$ in $s$,  when $a$ and $\bar{a}$ are both applicable in $s$ and have  the same qualitative effects over the features,
then soundness means that an   applicable abstract action can always be instantiated by
a  concrete action, while completeness  means that an applicable concrete action is always captured 
by an  applicable abstract action.  
...
An important intuition is that the features $F$ support a set of sound abstraction action $A_F$ over $F$ when  the
effects of the concrete actions over $F$  can be \emph{predicted} in the following sense:  there are no pairs of
state transitions $(s,s')$ and $(t,t')$  over instances in $\Q$ where a feature $f$ in $F$ changes in  different
ways (e.g., $f$ becomes true in one transition  but not in the other  or $f$ increases in one transition  but not in the other), 
when all the  features start with the same values in $s$ and $t$ (i.e., $\bar{s}=\bar{t}$).
This intuition will be made explicit  in the SAT encoding below  when enforcing 
a  notion  of soundness and completeness over sampled sets of transitions.

\begin{example}
  The set of  abstract actions  $\bar{a}=\abst{\neg H,n(x)>0}{H,n(x)\mminus}$
  and $\bar{a'}=\abst{H}{\neg H}$ over the features  $F=\{H,n(x)\}$
  is sound but not complete relative to $\Q_{clear}$. 
  This captures the concrete actions that pick blocks from above $x$ and puts them away
  but no  other actions like stacking a block above $x$ or picking up a block that is not above $x$. \qed
\end{example}
...
values in $s$ and $t$; namely, the  boolean features $p$ with different
...
into a constrained optimization problem, and more specifically, a Weighted Max-SAT problem,
where clauses resulting from the theories $T$ or $T_G$ are taken as \emph{hard} clauses
and \emph{soft} unit clauses $\neg selected(f)$ are added with  weights $cost(f)$
for all $f \in \F$.
...
G. Franc\`{e}s is funded by the European Research Council project ``State Space Exploration: Principles, Algorithms and Applications''.
