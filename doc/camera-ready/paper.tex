...
It is not fully clear whether it is reasonable in general
to assume that the features can be obtained from a large
pool of candidate features that is defined from the primitive
predicates and a general concept grammar.
...
that appear in the goal, are added to the set of primitive predicates and evaluated in the goal
\cite{martin-geffner:generalized}. 

Finally, while we have dealt with instances with deterministic actions
only, the formulation can be extended to handle non-deterministic actions.
Indeed, it is sufficient to replace the formula \eqref{eq:bridge1}
that links $D1$ and $D2$ with a formula to establish that when states
