...
The goal marking scheme that  yields the smaller  theory $T_G(\S}, \F)$ is obtained an optimal plan over
...
% @{\extracolsep{3pt}}
 ...
 & $I$ & $\abs{\S}$ & $k$ & $\abs{\F^k}$ & \multicolumn{2}{c}{$T$} & \multicolumn{2}{c}{$T_G$} &  $C_{\text{SAT}}$ & $\abs{F}$ & $\abs{A_F}$ & $\abs{I_F}$  & $\abs{G_F}$ & $\abs{Q^+_F}$ & $C_{\text{FOND}}$ \\ %
