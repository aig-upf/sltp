...
\begin{example}
  ...
  Blocksworld instances with stack and unstack actions with goal $clear(x)$.
...
\begin{enumerate}[{\small$\bullet$}]
\item Boolean $b_p$ for each primitive predicate $p$ of arity $0$ that is true iff $p^s$ is true,
...
if they are sound on the states $s$ that are reachable with the policy. We will see an example of this below.
...
generated by  a breadth-first search  along with  the states generated in an optimal plan. This is done for each
...
and stack/unstack actions. The primitive predicates, i.e., those appearing in the instances of $\Q_{clear}$, 
are assumed to use the predicates $on(\cdot,\cdot)$, $clear(\cdot)$,
...
\begin{enumerate}[--]
  \item $H: holding$ (holding a block),
  \item $X: \abs{holding \sqcap C_x}$ (holding block $x$),
  \item $n(x): \abs{\exists\,on^* . \set{x}}$, (number of blocks above block $x$).
\end{enumerate}
...
The primitive predicates are the same as those above.
For this problem, three training instances $P$ were used.
The set of $F$ obtained from $T_G$ contains
% 
...
while $E$ is $handempty$ and $G$ stands for $(\exists\,on^{-1}.C_x)  \sqcap C_y$.
...
The set $A_F$ includes actions to pick up the clear
...
\begin{enumerate}[--]
  ...
  \item $B: \exists\,at . \neg C_x$ (whether balls not in target room),
  \item $C: |\exists\,carry . \top|$ (number of balls carried),
...
The STRIPS instances have the primitive predicates $reward(c)$, $at(c)$, $blocked(c)$
...
We conclude the section with few observations.
...
on the number of disks begin even or  odd, which is not a feature
