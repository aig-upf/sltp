...
class Parser:
  ...
  def translate_to_atomic(self):
    ...
    print 'Setting h values'
    task.set_h_values(debug)
    print 'Setting max card'
    task.set_maximum_cardinality(debug)
    print 'Setting relevant actions'
    task.set_relevant_actions(debug)
    print 'Setting splitting'
    task.initialize_splitting(debug)
    task.set_relevant_actions_split(debug)
    start = timer()
...
# p = Parser()
# # p.set_domain('./F-bench/tireworld/domain.pddl')
# # p.set_problem('./F-bench/tireworld/p01.pddl')
# p.set_domain('./D-bench/blocks/domain.pddl')
# p.set_problem('./D-bench/blocks/task07.pddl')
# p.generate_file()
# p.generate_task()
